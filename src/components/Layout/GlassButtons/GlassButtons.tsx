'use client';

import React, { useState, useEffect, useRef } from 'react';
import styles from './GlassButtons.module.css';

const GlassButtons = () => {
  const [theme, setTheme] = useState<'light' | 'dark' | 'dim'>('light');
  const [mounted, setMounted] = useState(false);
  const toggleRef = useRef<HTMLDivElement>(null);
  const optionsRef = useRef<HTMLDivElement>(null);

  // Инициализация темы из localStorage или предпочтений системы
  useEffect(() => {
    setMounted(true);
    
    // Проверяем сохраненную тему
    const savedTheme = localStorage.getItem('theme') as 'light' | 'dark' | 'dim' | null;
    const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
    
    let initialTheme: 'light' | 'dark' | 'dim' = 'light';
    
    if (savedTheme) {
      initialTheme = savedTheme;
    } else if (prefersDark) {
      initialTheme = 'dark';
    }
    
    setTheme(initialTheme);
    applyTheme(initialTheme);
    
    // Позиционируем переключатель
    setTimeout(() => {
      positionToggle();
    }, 100);
  }, []);

  // Применение темы
  const applyTheme = (selectedTheme: 'light' | 'dark' | 'dim') => {
    const root = document.documentElement;
    
    // Обновляем CSS переменные в зависимости от темы
    switch (selectedTheme) {
      case 'dark':
        root.style.setProperty('--c-content', '#F5F5F5');
        root.style.setProperty('--c-glass', '#2A2A2A');
        root.style.setProperty('--c-light', '#FFFFFF');
        root.style.setProperty('--c-dark', '#000000');
        root.style.setProperty('--glass-reflex-light', '0.8');
        root.style.setProperty('--glass-reflex-dark', '1');
        root.style.setProperty('--saturation', '1.1');
        root.setAttribute('data-theme', 'dark');
        break;
      case 'dim':
        root.style.setProperty('--c-content', '#CCCCCC');
        root.style.setProperty('--c-glass', '#333333');
        root.style.setProperty('--c-light', '#DDDDDD');
        root.style.setProperty('--c-dark', '#111111');
        root.style.setProperty('--glass-reflex-light', '0.9');
        root.style.setProperty('--glass-reflex-dark', '0.9');
        root.style.setProperty('--saturation', '1');
        root.setAttribute('data-theme', 'dim');
        break;
      case 'light':
      default:
        root.style.setProperty('--c-content', '#1F1F1F');
        root.style.setProperty('--c-glass', '#FFFFFF');
        root.style.setProperty('--c-light', '#000000');
        root.style.setProperty('--c-dark', '#FFFFFF');
        root.style.setProperty('--glass-reflex-light', '1');
        root.style.setProperty('--glass-reflex-dark', '0.8');
        root.style.setProperty('--saturation', '1');
        root.setAttribute('data-theme', 'light');
        break;
    }
    
    // Обновляем body
    document.body.className = selectedTheme;
    
    // Обновляем отмеченный radio button
    const radioButtons = document.querySelectorAll('input[name="theme"]');
    radioButtons.forEach((radio: any) => {
      if (radio.value === selectedTheme) {
        radio.checked = true;
      }
    });
  };

  // Позиционирование переключателя
  const positionToggle = () => {
    const toggle = toggleRef.current;
    const options = optionsRef.current;
    
    if (!toggle || !options) return;
    
    const activeOption = options.querySelector(`input[value="${theme}"]`);
    if (!activeOption) return;
    
    const optionRect = (activeOption as HTMLElement).getBoundingClientRect();
    const optionsRect = options.getBoundingClientRect();
    
    toggle.style.setProperty('--x', `${optionRect.left - optionsRect.left}px`);
    toggle.style.setProperty('--w', `${optionRect.width}px`);
  };

  // Обработчик изменения темы
  const handleThemeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newTheme = e.target.value as 'light' | 'dark' | 'dim';
    setTheme(newTheme);
    localStorage.setItem('theme', newTheme);
    applyTheme(newTheme);
    
    // Позиционируем переключатель с задержкой для анимации
    setTimeout(() => {
      positionToggle();
    }, 50);
  };

  if (!mounted) return null;

  return (
    <>
      {/* SVG фильтр для эффекта жидкого стекла */}
      <svg style={{ display: 'none' }}>
        <defs>
          <filter id="switcher" primitiveUnits="objectBoundingBox">
            <feImage 
              result="map" 
              width="100%" 
              height="100%" 
              x="0" 
              y="0" 
              href="data:image/webp;base64," 
            />
            <feGaussianBlur in="SourceGraphic" stdDeviation="0.04" result="blur" />
            <feDisplacementMap 
              id="disp" 
              in="blur" 
              in2="map" 
              scale="0.5" 
              xChannelSelector="R" 
              yChannelSelector="G"
            />
          </filter>
          <filter id="toggler" primitiveUnits="objectBoundingBox">
            <feImage 
              result="map" 
              width="100%" 
              height="100%" 
              x="0" 
              y="0" 
              href="data:image/webp;base64,"
            />
            <feGaussianBlur in="SourceGraphic" stdDeviation="0.01" result="blur" />
            <feDisplacementMap 
              id="disp" 
              in="blur" 
              in2="map" 
              scale="0.5" 
              xChannelSelector="R" 
              yChannelSelector="G"
            />
          </filter>
        </defs>
      </svg>

      {/* Основной контейнер переключателя */}
      <fieldset className={styles.switcher}>
        <legend className={styles.switcher__legend}>Choose theme</legend>
        
        <div className={styles.switcher__options} ref={optionsRef}>
          {/* Светлая тема */}
          <label className={`${styles.switcher__option} ${theme === 'light' ? styles.active : ''}`}>
            <input 
              className={styles.switcher__input} 
              type="radio" 
              name="theme" 
              value="light" 
              data-option="1"
              checked={theme === 'light'}
              onChange={handleThemeChange}
            />
            <svg className={styles.switcher__icon} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 36 36">
              <path fill="var(--c)" fillRule="evenodd" d="M18 12a6 6 0 1 1 0 12 6 6 0 0 1 0-12Zm0 2a4 4 0 1 0 0 8 4 4 0 0 0 0-8Z" clipRule="evenodd" />
              <path fill="var(--c)" d="M17 6.038a1 1 0 1 1 2 0v3a1 1 0 0 1-2 0v-3ZM24.244 7.742a1 1 0 1 1 1.618 1.176L24.1 11.345a1 1 0 1 1-1.618-1.176l1.763-2.427ZM29.104 13.379a1 1 0 0 1 .618 1.902l-2.854.927a1 1 0 1 1-.618-1.902l2.854-.927ZM29.722 20.795a1 1 0 0 1-.619 1.902l-2.853-.927a1 1 0 1 1 .618-1.902l2.854.927ZM25.862 27.159a1 1 0 0 1-1.618 1.175l-1.763-2.427a1 1 0 1 1 1.618-1.175l1.763 2.427ZM19 30.038a1 1 0 0 1-2 0v-3a1 1 0 1 1 2 0v3ZM11.755 28.334a1 1 0 0 1-1.618-1.175l1.764-2.427a1 1 0 1 1 1.618 1.175l-1.764 2.427ZM6.896 22.697a1 1 0 1 1-.618-1.902l2.853-.927a1 1 0 1 1 .618 1.902l-2.853.927ZM6.278 15.28a1 1 0 1 1 .618-1.901l2.853.927a1 1 0 1 1-.618 1.902l-2.853-.927ZM10.137 8.918a1 1 0 0 1 1.618-1.176l1.764 2.427a1 1 0 0 1-1.618 1.176l-1.764-2.427Z" />
            </svg>
          </label>
          
          {/* Темная тема */}
          <label className={`${styles.switcher__option} ${theme === 'dark' ? styles.active : ''}`}>
            <input 
              className={styles.switcher__input} 
              type="radio" 
              name="theme" 
              value="dark" 
              data-option="2"
              checked={theme === 'dark'}
              onChange={handleThemeChange}
            />
            <svg className={styles.switcher__icon} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 36 36">
              <path fill="var(--c)" d="M12.5 8.473a10.968 10.968 0 0 1 8.785-.97 7.435 7.435 0 0 0-3.737 4.672l-.09.373A7.454 7.454 0 0 0 28.732 20.4a10.97 10.97 0 0 1-5.232 7.125l-.497.27c-5.014 2.566-11.175.916-14.234-3.813l-.295-.483C5.53 18.403 7.13 11.93 12.017 8.77l.483-.297Zm4.234.616a8.946 8.946 0 0 0-2.805.883l-.429.234A9 9 0 0 0 10.206 22.5l.241.395A9 9 0 0 0 22.5 25.794l.416-.255a8.94 8.94 0 0 0 2.167-1.99 9.433 9.433 0 0 1-2.782-.313c-5.043-1.352-8.036-6.535-6.686-11.578l.147-.491c.242-.745.573-1.44.972-2.078Z" />
            </svg>
          </label>
          
          {/* Dim тема */}
          <label className={`${styles.switcher__option} ${theme === 'dim' ? styles.active : ''}`}>
            <input 
              className={styles.switcher__input} 
              type="radio" 
              name="theme" 
              value="dim" 
              data-option="3"
              checked={theme === 'dim'}
              onChange={handleThemeChange}
            />
            <svg className={styles.switcher__icon} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 36 36">
              <path fill="var(--c)" d="M5 21a1 1 0 0 1 1-1h24a1 1 0 1 1 0 2H6a1 1 0 0 1-1-1ZM12 25a1 1 0 0 1 1-1h10a1 1 0 1 1 0 2H13a1 1 0 0 1-1-1ZM15 29a1 1 0 0 1 1-1h4a1 1 0 1 1 0 2h-4a1 1 0 0 1-1-1ZM18 13a6 6 0 0 1 5.915 7h-2.041A4.005 4.005 0 0 0 18 15a4 4 0 0 0-3.874 5h-2.041A6 6 0 0 1 18 13ZM17 7.038a1 1 0 1 1 2 0v3a1 1 0 0 1-2 0v-3ZM24.244 8.742a1 1 0 1 1 1.618 1.176L24.1 12.345a1 1 0 1 1-1.618-1.176l1.763-2.427ZM29.104 14.379a1 1 0 0 1 .618 1.902l-2.854.927a1 1 0 1 1-.618-1.902l2.854-.927ZM6.278 16.28a1 1 0 1 1 .618-1.901l2.853.927a1 1 0 1 1-.618 1.902l-2.853-.927ZM10.137 9.918a1 1 0 0 1 1.618-1.176l1.764 2.427a1 1 0 0 1-1.618 1.176l-1.764-2.427Z" />
            </svg>
          </label>
        </div>
        
        {/* Анимированный переключатель */}
        <div 
          className={styles.switcher__toggle}
          ref={toggleRef}
          style={{
            '--x': '0px',
            '--w': '68px'
          } as React.CSSProperties}
        />
        
        {/* Фильтр для эффекта стекла */}
        <div className={styles.switcher__filter}>
          <svg>
            {/* Фильтры уже определены выше */}
          </svg>
        </div>
      </fieldset>
    </>
  );
};

export default GlassButtons;