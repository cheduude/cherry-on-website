...
const Auth: React.FC = () => {
  ...
  const handleTelegramAuth = useCallback((user: any) => {
    setIsSubmitting(true);
    try {
      const name = `${user.first_name} ${user.last_name || ''}`.trim() || user.username || 'Telegram User';
      const email = user.username ? `${user.username}@telegram.com` : `id${user.id}@telegram.com`;
      const avatar = user.photo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=random&color=fff&size=32&bold=true`;
